...
### `serve-pdf [--host <호스트>] [--port <포트>]`
HWP/HWPX/HML 문서 바이트를 PDF로 변환하는 HTTP API 서버를 실행한다.
- 기본 수신 주소는 `127.0.0.1:7878`이다. 컨테이너에서는 `--host 0.0.0.0`을 사용한다.
- `POST /api/convert/hwp-to-pdf` 요청 본문에 원본 문서 바이트를 그대로 보낸다.
  성공 응답은 `Content-Type: application/pdf`인 PDF 바이트다.
- `?page=<번호>`로 0-based 단일 페이지를 선택할 수 있다. 생략하면 전체 문서를 변환한다.
- 요청 본문은 `Content-Length`가 필수이며 최대 256 MiB다.
- 변환할 수 없는 문서는 `422`, 잘못된 요청은 `400` 계열 상태로 응답한다.

```bash
rhwp serve-pdf --host 0.0.0.0 --port 7878
curl --data-binary @input.hwp \
  -H 'Content-Type: application/octet-stream' \
  'http://127.0.0.1:7878/api/convert/hwp-to-pdf' \
  -o output.pdf
```
