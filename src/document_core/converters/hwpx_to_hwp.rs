...
/// `source_format` 검사 후 XML/HWP3 출처(HWPX/HWPML/HWP3)에만 어댑터를 호출하는 보조 함수.
///
...
pub fn convert_if_hwpx_source(doc: &mut Document, source_format: FileFormat) -> AdapterReport {
    if !matches!(
        source_format,
        FileFormat::Hwpx | FileFormat::Hwpml | FileFormat::Hwp3
    ) {
        return AdapterReport::new().no_op("source_format != Hwpx|Hwpml|Hwp3");
    }
...
mod tests {
    ...
    fn hwp_source_no_op_via_filter() {
        ...
        assert_eq!(report.skipped_reason.as_deref(), Some("source_format != Hwpx|Hwpml|Hwp3"));
    }
