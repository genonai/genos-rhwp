...
fn main() {
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        Some("serve-pdf") => exit_with(serve_pdf_api(&args[2..])),
...
fn capabilities_command_entries() -> Vec<serde_json::Value> {
    let mut commands = vec![
        ...
        cmd(
            "serve-pdf",
            "serve",
            "HWP/HWPX/HML 문서 바이트를 PDF로 변환하는 HTTP API 서버",
        ),
        cmd_json(
            "export-markdown",
            "export",
...
fn print_help() {
    ...
    println!();
    println!("  serve-pdf [--host <호스트>] [--port <포트>]");
    println!("      HWP/HWPX/HML -> PDF 변환 HTTP API 서버 실행");
    println!("      POST /api/convert/hwp-to-pdf 로 application/octet-stream 문서 바이트 전송");
    println!("      ?page=<번호>로 특정 페이지만 변환 (0부터 시작)");
    println!();
...
fn serve_pdf_api(args: &[String]) -> i32 {
    #[cfg(target_arch = "wasm32")]
    {
        let _ = args;
        eprintln!("오류: PDF API 서버는 네이티브 빌드에서만 사용할 수 있습니다.");
        return EXIT_USAGE;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        ...
        while i < args.len() {
            match args[i].as_str() {
                "--host" => match args.get(i + 1) {
                    Some(value) => {
                        host = value.clone();
                        i += 2;
                    }
                    None => {
                        eprintln!("오류: --host 뒤에 호스트가 필요합니다.");
                        return EXIT_USAGE;
                    }
                },
                "--port" | "-p" => match args.get(i + 1) {
                    Some(value) => {
                        port = value.clone();
                        i += 2;
                    }
                    None => {
                        eprintln!("오류: --port 뒤에 포트가 필요합니다.");
                        return EXIT_USAGE;
                    }
                },
                other => {
                    eprintln!("오류: 알 수 없는 옵션 {other}");
                    eprintln!("사용법: rhwp serve-pdf [--host 127.0.0.1] [--port 7878]");
                    return EXIT_USAGE;
                }
            }
        }
        ...
        let listener = match TcpListener::bind(&addr) {
            ...
            Err(error) => {
                eprintln!("오류: {addr} 바인딩 실패 - {error}");
                return EXIT_RUNTIME;
            }
        };

        println!("PDF 변환 API 서버 시작: http://{addr}");
        println!("POST /api/convert/hwp-to-pdf  Content-Type: application/octet-stream");

        for stream in listener.incoming() {
            match stream {
                Ok(stream) => {
                    std::thread::spawn(move || {
                        if let Err(error) = handle_pdf_api_connection(stream) {
                            eprintln!("요청 처리 실패: {error}");
                        }
                    });
                }
                Err(error) => eprintln!("연결 실패: {error}"),
            }
        }
        EXIT_OK
    }
...
#[cfg(not(target_arch = "wasm32"))]
fn handle_pdf_api_connection(mut stream: TcpStream) -> Result<(), String> {
    ...
    let header_end = loop {
        ...
        if count == 0 {
            ...
            return Err("요청 헤더가 완성되기 전에 연결이 종료되었습니다".to_string());
        }
    ...
    if path != "/api/convert/hwp-to-pdf" {
        write_response(
            &mut stream,
            "404 Not Found",
            "text/plain; charset=utf-8",
    ...
    let target_page = match query_page(query) {
        Ok(page) => page,
        Err(error) => {
            write_response(
                &mut stream,
    ...
    match document_to_pdf_bytes(&body, target_page) {
        Ok((pdf_bytes, _)) => {
            write_response(&mut stream, "200 OK", "application/pdf", &pdf_bytes)?;
        }
        Err(error) => {
            write_response(
                &mut stream,
...
#[cfg(not(target_arch = "wasm32"))]
fn document_to_pdf_bytes(
    data: &[u8],
    target_page: Option<u32>,
) -> Result<(Vec<u8>, usize), String> {
    let doc = rhwp::wasm_api::HwpDocument::from_bytes(data)
        .map_err(|e| format!("문서 파싱 실패: {e}"))?;
    let page_count = doc.page_count();
    if page_count == 0 {
        return Err("PDF로 내보낼 페이지가 없습니다.".to_string());
    }
    let pages = match target_page {
...
#[cfg(not(target_arch = "wasm32"))]
fn query_page(query: Option<&str>) -> Result<Option<u32>, String> {
    ...
    for pair in query.split('&') {
        ...
        if key == "page" {
            return value
                .parse::<u32>()
                .map(Some)
                .map_err(|_| "page query must be an unsigned integer".to_string());
        }
    }
    Ok(None)
}
...
mod tests {
    use super::{
        ...
        collect_audit_capsules, content_length, query_page, replay_scratch_dir,
        set_cli_output_password, set_cli_password, split_target, strip_global_auth_options,
    ...
    #[test]
    fn pdf_api_request_target_and_page_query_are_parsed() {
        assert_eq!(
            split_target("/api/convert/hwp-to-pdf?page=2"),
            ("/api/convert/hwp-to-pdf", Some("page=2"))
        );
        assert_eq!(query_page(Some("page=2")).unwrap(), Some(2));
        assert_eq!(query_page(Some("foo=bar")).unwrap(), None);
        assert!(query_page(Some("page=-1")).is_err());
    }

    #[test]
    fn pdf_api_content_length_is_case_insensitive() {
        assert_eq!(
            content_length("Host: localhost\r\nContent-Length: 42\r\n"),
            Some(42)
        );
        assert_eq!(content_length("content-length: 7\r\n"), Some(7));
        assert_eq!(content_length("Host: localhost\r\n"), None);
    }
