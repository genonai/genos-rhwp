...
fn main() {
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        Some("serve-pdf") => serve_pdf_api(&args[2..]),
...
fn print_help() {
    ...
    println!("      HWP→PDF 변환 HTTP API 서버 실행");
    println!("      POST /api/convert/hwp-to-pdf 로 application/octet-stream HWP 바이트 전송");
    println!();
...
fn hwp_to_pdf_bytes(data: &[u8], target_page: Option<u32>) -> Result<(Vec<u8>, usize), String> {
    ...
    for page_num in &pages {
        let svg = doc
            .render_page_svg(*page_num)
            .map_err(|e| format!("페이지 {} 렌더링 실패: {:?}", page_num, e))?;
        svg_pages.push(svg);
    }

    rhwp::renderer::pdf::svgs_to_pdf(&svg_pages).map(|bytes| (bytes, svg_pages.len()))
}
...
fn export_pdf(args: &[String]) {
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        match hwp_to_pdf_bytes(&data, target_page) {
            Ok((pdf_bytes, converted_pages)) => match fs::write(&output_file, &pdf_bytes) {
                Ok(_) => println!(
                    "  → {} ({}KB, {}페이지)",
                    output_file,
                    pdf_bytes.len() / 1024,
            ...
            match stream {
                Ok(stream) => {
                    std::thread::spawn(move || {
                        if let Err(e) = handle_pdf_api_connection(stream) {
                            eprintln!("요청 처리 실패: {}", e);
                        }
                    });
                }
...
fn handle_pdf_api_connection(mut stream: TcpStream) -> Result<(), String> {
    ...
    match hwp_to_pdf_bytes(&body, target_page) {
        Ok((pdf_bytes, _page_count)) => {
